  ...
  <div class="page-container">
    ...
    <div class="content-wrapper">
      <header class="page-header flex-between">
        ...
        <button @click="openManualEntry" class="btn btn-primary">+ Manual Entry</button>
      </header>
        ...
        <div class="card-body">
          <div class="filter-row">
            <div class="filter-group">
              ...
              <select v-model="filters.employee_id" class="form-input" @change="loadEntries">
            ...
            <div class="filter-group">
              ...
              <input type="date" v-model="filters.start_date" class="form-input" @change="loadEntries" />
            </div>
            <div class="filter-group">
              <label>End Date</label>
              <input type="date" v-model="filters.end_date" class="form-input" @change="loadEntries" />
            </div>
                ...
                <tr v-for="entry in entries" :key="entry.id">
                  <td>
                    <div>
                      ...
                      <div class="text-muted text-mono" style="font-size: 0.8rem">{{ entry.employee_pin }}</div>
                    </div>
                  </td>
                  ...
                  <td>
                    <span v-if="entry.clock_out">{{ formatDuration(entry.total_minutes) }}</span>
                    <span v-else class="badge badge-success">Active</span>
                  </td>
                  <td>{{ entry.notes || '-' }}</td>
                  <td>
                    <div class="action-buttons">
                      <button @click="openEditEntry(entry)" class="btn btn-outline btn-sm">Edit</button>
                      <button @click="confirmDelete(entry)" class="btn btn-outline btn-sm text-error">Delete</button>
                    </div>
                  </td>
                </tr>
              </tbody>
        ...
        <form @submit.prevent="saveEntry">
          <div class="modal-body">
            ...
            <div v-if="!editingEntry" class="form-group">
              ...
              <select v-model="entryForm.employee_id" class="form-input" required>
            ...
            <div class="form-group">
              ...
              <input type="datetime-local" v-model="entryForm.clock_in" class="form-input" required />
            </div>
            
            <div class="form-group">
              <label class="form-label">Clock Out *</label>
              <input type="datetime-local" v-model="entryForm.clock_out" class="form-input" required />
            </div>
            
            <div class="form-group">
              <label class="form-label">Notes</label>
              <textarea v-model="entryForm.notes" class="form-input" rows="3"></textarea>
            </div>
          </div>
    ...
    <div v-if="showDeleteModal" class="modal-overlay" @click.self="showDeleteModal = false">
      <div class="modal">
        ...
        <div class="modal-footer">
          ...
          <button @click="deleteEntry" class="btn btn-primary" style="background: var(--error)">Delete</button>
        </div>
      </div>
    </div>
  </div>
